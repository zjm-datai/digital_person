...
def _get_cache_configuration() -> CacheConfig | None:
    ...
    if not app_config.REDIS_ENABLE_CLIENT_SIDE_CACHE:
        return None

    resp_protocol = app_config.REDIS_SERIALIZATION_PROTOCOL
    if resp_protocol < 3:
...
def _create_sentinel_client(redis_params: dict[str, Any]) -> Union[redis.Redis, RedisCluster]:
    ...
    if not app_config.REDIS_SENTINELS:
        raise ValueError("REDIS_SENTINELS must be set when REDIS_USE_SENTINEL is True")

    if not app_config.REDIS_SENTINEL_SERVICE_NAME:
        raise ValueError("REDIS_SENTINEL_SERVICE_NAME must be set when REDIS_USE_SENTINEL is True")

    sentinel_hosts = [(node.split(":")[0], int(node.split(":")[1])) for node in app_config.REDIS_SENTINELS.split(",")]
    ...
    master: redis.Redis = sentinel.master_for(app_config.REDIS_SENTINEL_SERVICE_NAME, **redis_params)
    return master


def _create_cluster_client() -> Union[redis.Redis, RedisCluster]:
    """Create Redis cluster client."""
    if not app_config.REDIS_CLUSTERS:
    ...
    cluster: RedisCluster = RedisCluster(
        startup_nodes=nodes,
        password=app_config.REDIS_CLUSTERS_PASSWORD,
        protocol=app_config.REDIS_SERIALIZATION_PROTOCOL,
        cache_config=_get_cache_configuration(),
    )
    return cluster
